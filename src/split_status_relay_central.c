...
static uint8_t central_state_of_charge = 0xff;
static uint8_t highest_active_layer = 0xff;
...
static void send_all_events(uint32_t initial_delay_ms) {
    ...
    for (uint8_t i = 0; i < CONFIG_ZMK_SPLIT_BLE_CENTRAL_PERIPHERALS; i++) {
        send_connection_state_event(i, peripheral_connections[i].connected, i == 0 ? initial_delay_ms : 0);
        if (peripheral_connections[i].connected && peripheral_connections[i].battery_level != 0xFF) {
            send_peripheral_battery_level_event(i, peripheral_connections[i].battery_level, initial_delay_ms);
        }
    }

    if (central_state_of_charge != 0xff) {
        send_central_battery_level_event(central_state_of_charge, 0);
    }

    if (highest_active_layer != 0xff) {
        send_active_layer_event(highest_active_layer, 0);
    }
}
...
static int central_layer_listener(const zmk_event_t *eh) {
    ...
    if (layer == highest_active_layer) {
        // no change
        return ZMK_EV_EVENT_BUBBLE;
    }
    highest_active_layer = layer;
...
static int srcc_init(const struct device *dev) {
    ...
    highest_active_layer = zmk_keymap_highest_layer_active();
